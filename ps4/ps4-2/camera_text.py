...
def process_camera(c_id, t_pos):
    ...
    camera = iort.write_camera_2d(c_id, t_pos, ts, 'cam_view_0.jpg') # change file name in PS4
    c_key = camera['c_key']
    ...
    #Add the position recognizing code
    Red, Green, img = Camera.DetectParts(image)
    obj = [] 
    for G in Green:
        obj.append({u'pos_x': G[0], u'pos_y': G[1], u'o_type': u'G', u'c_key': c_key, u'u_name': 0, u'aux': 1, u'rot': G[2]})
    for R in Red:
        obj.append({u'pos_x': R[0], u'pos_y': R[1], u'o_type': u'R', u'c_key': c_key, u'u_name': 0, u'aux': 1, u'rot': R[2]})
